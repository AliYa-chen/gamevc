...
<template>
  <aside class="lg:sticky lg:top-4 lg:h-[calc(100svh-2rem)]">
    <!-- 外壳 -->
    <div
      class="relative h-full rounded-3xl border border-white/10
             bg-white/5 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]
             backdrop-blur-2xl overflow-hidden
             transition-all duration-300"
      :class="game.loading && 'opacity-60 saturate-50'"
    >
      <!-- 背景动态层 -->
      <div class="pointer-events-none absolute inset-0 -z-10 overflow-hidden rounded-3xl">
        <div
          class="absolute -left-4 -top-4 h-64 w-64 rounded-full
                 bg-purple-500/20 blur-3xl
                 animate-blob mix-blend-screen"
        />
        <div
          class="absolute -right-4 top-0 h-64 w-64 rounded-full
                 bg-blue-500/20 blur-3xl
                 animate-blob animation-delay-2000 mix-blend-screen"
        />
        <div
          class="absolute -bottom-8 left-20 h-64 w-64 rounded-full
      ...
      <!-- 内容动画层 -->
      <div
        class="relative flex h-full flex-col gap-8 p-6 sm:p-8
      ...
      >
        ...
        <!-- 设置面板 -->
        <div
          class="rounded-2xl border border-white/15 bg-black/50
